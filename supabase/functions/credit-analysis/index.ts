...
const API_CREDIT_ANALYSIS_URL =
  Deno.env.get("API_CREDIT_ANALYSIS_URL") ||
  "https://uat-api.serasaexperian.com.br/consultas/v1/relato"
const API_CREDIT_ANALYSIS_TOKEN = Deno.env
  .get("API_CREDIT_ANALYSIS_TOKEN")
  ?.trim()
const API_CREDIT_ANALYSIS_AUTH_HEADER =
  Deno.env.get("API_CREDIT_ANALYSIS_AUTH_HEADER")?.trim() || "Authorization"
const API_CREDIT_ANALYSIS_AUTH_SCHEME =
  Deno.env.get("API_CREDIT_ANALYSIS_AUTH_SCHEME")?.trim() || "Bearer"
const USE_MOCK_CREDIT_ANALYSIS =
  Deno.env.get("USE_MOCK_CREDIT_ANALYSIS") === "true"
...
function buildExternalHeaders(): HeadersInit {
  ...
  const shouldFormatAsAuthorization =
    API_CREDIT_ANALYSIS_AUTH_HEADER.toLowerCase() === "authorization"
  const authValue =
    shouldFormatAsAuthorization && API_CREDIT_ANALYSIS_AUTH_SCHEME
  ...
  try {
    ...
    if (score === null) {
      ...
      return createJsonResponse(
        { error: "Resposta inválida da API de crédito" },
        502,
      )
    }
    ...
    return createJsonResponse(
      { error: "Falha na comunicação com serviço de crédito" },
      503,
    )
  }
})
